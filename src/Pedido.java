...
public class Pedido implements Comparable<Pedido>{
     ...
     */
		...
		for (int i = 0; i < quantItens; i++) {
			valorPedido += itens[i].getProduto().valorDeVenda() * itens[i].getQuantidade();
		}

		if (formaDePagamento == 1) { // Pagamento a vista
			valorPedido = valorPedido * (1.0 - DESCONTO_PG_A_VISTA);
		}

		BigDecimal valorPedidoBD = new BigDecimal(valorPedido).setScale(2, RoundingMode.HALF_UP);
